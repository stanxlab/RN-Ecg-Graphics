...
export default class App extends Component {
    ...
    handleMessage(e) {
        ...
        console.info('RN收到前端的数据: ', msg);
    }

    sendMsgToHtml() {
        let data = {
            time: Date.now(),
            remark: 'data-from-RN-init',
        }
        let msg = JSON.stringify(data);
        console.info('向前端发送数据: ', msg);
        this.webView.postMessage(msg);
    }
        ...
        return (
            <View style={{
                ...
                <View style={{
                    ...
                    <Text> 下面嵌入一个WebView,并通过RN发送数据给html绘制图形.</Text>
                </View>
                <WebView
                    ...
                    originWhitelist={["*"]}
                    // 开发时可以使用 http 请求html页面, 发布则可以将html文本嵌入到js代码中
                    source={{ html: htmlTxt }}
                    // source={{ uri: "http://10.141.107.148:8001/test.html" }}
